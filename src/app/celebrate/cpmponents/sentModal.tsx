        ...
        {currentStep === 1 && (
          <>
            ...
            <div className="flex flex-row justify-center gap-3 items-center m-3">
              <h2 className="text-sm z-30 bg-yellow-100 rounded-3xl p-2">step1</h2>
            ...
            <div className="flex flex-row justify-center gap-3 items-center m-3">
              <h2 className="text-sm z-30 bg-yellow-100 rounded-3xl p-2">step2</h2>
              <h2 className="text-xl z-30 animate-tracking-in-expand">お祝いカードを選択</h2>
            </div>
            ...
            <div className="flex flex-row gap-2 m-2 mb-6">
              {images.map((image, index) => (
                <div
                  key={index}
                  className={`border-2 rounded ${selectedImage === image ? 'border-yellow-400' : 'border-transparent'}`}
                  onClick={() => handleImageSelect(image)}
                >
                  <Image src={image} alt={`Background ${index + 1}`} className="w-[30px] h-[30px] cursor-pointer" />
                </div>
              ))}
            </div>
                ...
                {selectedImage && (
                  <div className="w-[280px] h-[350px]">
                    ...
                    <div className="absolute inset-0 flex flex-col justify-center  text-center rounded">
                      <div className="mb-9 text-xl ">{newPostTitle}</div>
                      <div className="m-8 text-xl ">{newPostContent}</div>
                    </div>
                  </div>
                )}
              </div>
